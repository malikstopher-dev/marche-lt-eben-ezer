...
export default function ProductEditPage() {
  ...
  const productIdFromParams = params.id as string;
  const productIdFromQuery = searchParams.get("id");
  
  const isNew = productIdFromParams === "new" || (!productIdFromQuery && productIdFromParams === "new");
  const productId = isNew ? null : (productIdFromQuery ? parseInt(productIdFromQuery) : parseInt(productIdFromParams));
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[#1A1A1A]">{isNew ? "Nouveau Produit" : "Modifier le Produit"}</h1>
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ex: 1 kg, 500ml, 12 x 290ml"
                    className="w-full px-4 py-2.5 border border-[#E8E4DD] rounded-xl text-[#1A1A1A] focus:ring-2 focus:ring-[#2D5A3D]"
                  />
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-[#E8E4DD] p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-[#1A1A1A] mb-2">Prix comparatif (CAD)</label>
                <input
                  type="number"
                  step="0.01"
          ...
          <div className="bg-white rounded-2xl border border-[#E8E4DD] p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <label className="block text-sm font-medium text-[#1A1A1A]">URL de l'image</label>
                <input
                  type="text"
                  value={product.image || ""}
              ...
              <div className="flex items-center gap-2">
                <label className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 border border-[#E8E4DD] rounded-xl text-[#1A1A1A] text-sm cursor-pointer hover:bg-[#F5F2ED] transition-colors">
                ...
                {(product.image && product.image !== PLACEHOLDER_IMAGE) && (
                  <button
                    ...
                    className="px-4 py-2.5 border border-[#C41E3A] text-[#C41E3A] rounded-xl text-sm hover:bg-[#C41E3A]/10 transition-colors"
                  >
                    Effacer
                  </button>
                )}
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              ...
              <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                <input
                  type="checkbox"
                  checked={product.price_needs_review}
                  onChange={(e) => setProduct({ ...product, price_needs_review: e.target.checked })}
                  className="w-5 h-5 rounded border-gray-300 text-amber-600 focus:ring-amber-500"
                />
                <div>
                  <p className="text-sm font-medium text-gray-900">Price Needs Review</p>
                  <p className="text-xs text-gray-500">Mark for price review</p>
                </div>
              </label>
            </div>
          </div>
            ...
            <div className="space-y-4">
              <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Show product in promotions section</p>
                </div>
              </label>
              {product.onPromo && (
                <div className="pl-3 space-y-4 border-l-2 border-red-200 ml-2">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="0.00"
                      className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-gray-900 focus:ring-2 focus:ring-red-500 focus:border-red-500"
                    />
                  </div>
                  <div>
            ...
            <div className="space-y-3">
              <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Product not available for purchase</p>
                </div>
              </label>
            </div>
          </div>
