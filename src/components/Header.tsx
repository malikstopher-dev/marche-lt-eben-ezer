      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <nav className="hidden lg:flex items-center gap-0.5">
            ...
            <div className="relative" ref={departementsRef}>
              ...
              {isDepartementsOpen && (
                <div className="absolute top-full left-0 mt-1 w-48 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden z-50">
          ...
          <form onSubmit={handleSearch} className="hidden md:flex items-center flex-1 max-w-md mx-4">
            <div className="relative w-full">
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Rechercher un produit..." className="w-full px-4 py-2 pl-10 bg-white/10 border border-white/10 rounded-full text-white placeholder-white/40 focus:outline-none focus:bg-white/15 focus:border-white/20 transition-colors text-sm" />
          ...
          <div className="flex items-center gap-1">
            ...
            {/* Promotions Button */}
            <Link href="/promotions" className="hidden sm:flex items-center px-3 py-1.5 bg-[#ec7205] text-white text-xs font-semibold rounded-full hover:bg-[#d66a05] transition-colors">
              Promos
            </Link>
            ...
            <Link href="/cart" className="relative p-2 text-white/80 hover:text-white transition-colors">
              ...
              {cartCount > 0 && (
                <span className="absolute -top-1 -right-1 w-5 h-5 bg-[#ec7205] text-white text-xs font-semibold rounded-full flex items-center justify-center">
                  {cartCount}
                </span>
              )}
            </Link>
        ...
        {isMobileSearchOpen && (
          <form onSubmit={handleSearch} className="md:hidden pb-4">
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Rechercher..." className="w-full px-4 py-2 bg-white/10 border border-white/10 rounded-full text-white placeholder-white/40 focus:outline-none text-sm" />
          </form>
        )}
